...
(function () {
  ...
  function captureUTMs() {
    ...
    if (hasFresh) {
      try { sessionStorage.setItem('gmpb_utms', JSON.stringify(fresh)); } catch (_) {}
      return fresh;
    }
    try {
      const stored = sessionStorage.getItem('gmpb_utms');
      if (stored) return JSON.parse(stored);
      sessionStorage.setItem('gmpb_utms', JSON.stringify(fresh));
    } catch (_) {}
    return fresh;
  }
  ...
  function captureClickIds() {
    ...
    CLICK_ID_KEYS.forEach((k) => {
      let v = params.get(k);
      if (!v) {
        try { v = sessionStorage.getItem('gmpb_' + k) || ''; } catch (_) { v = ''; }
      }
      if (v) {
        out[k] = v;
        try { sessionStorage.setItem('gmpb_' + k, v); } catch (_) {}
      }
    });
    return out;
  }
  ...
  window.GMPB_UTMS = UTMS;
  window.GMPB_CLICK_IDS = CLICK_IDS;
  window.GMPB_CUSTOM_DATA = function () {
    ...
    try {
      if (window.fbq) {
        window.fbq('track', 'InitiateCheckout', Object.assign(
          { content_name: 'guia-piano-mpb', content_category: 'piano-mpb-ebook' },
          UTMS, CLICK_IDS
        ));
      }
    } catch (_) {}
